...
(function ($)
{
	...
	$.ligerui.DialogImagePath = base + "/core_res/thirdparty/ligerUI/skins/Aqua/images/win/";
        ...
        loadData: function (loadDataParm)
        {
            ...
            if	(p.dataAction == "server" && p.columns.length) {
            	...
            	$(p.columns).each(function() {
					...
					if(this.dataFormat) { 
						dataFormat.push(this.dataFormat);
					}else{
						dataFormat.push("");
					}
            	});
				...
				param.push({ name: "dataFormat", value: dataFormat});
            }
            if (p.dataAction == "server")
            {
                if (p.usePager)
